...
export class UserLogController {
	...
	getAll = asyncHandler(async (req: Request, res: Response) => {
		...
		const { logs, total } = await this.service.getAllUserLogs(
			page,
			limit,
			sortBy,
			sortOrder,
	...
	getByUserId = asyncHandler(async (req: Request, res: Response) => {
		...
		const { logs, total } = await this.service.getUserLogsByUserId(
	...
	clearByUserId = asyncHandler(async (req: Request, res: Response) => {
		const result = await this.service.clearUserLogsByUserId(
			getRequiredParam(req, "userId"),
		);
		sendSuccessResponse(res, {
			data: result,
			message: "User logs cleared successfully",
		});
	});
