...
export const errorHandler = (
  ...
  if (error instanceof ZodError) {
    ...
    return sendErrorResponse(res, 'Please check your input and try again', 422, errors);
  }
  ...
  if (error.name === 'ValidationError') {
    return sendErrorResponse(res, 'The data provided is not valid. Please check and try again', 400);
  }

  if (error.name === 'CastError') {
    return sendErrorResponse(res, 'The ID provided is not valid. Please check and try again', 400);
  }

  if (error.name === 'MongoServerError' || ('code' in error && (error as { code: number }).code === 11000)) {
    return sendErrorResponse(res, 'A record with this information already exists', 409);
  }
