...
// Base schema helpers
export const createStringSchema = (required: boolean = true, minLength?: number, maxLength?: number) => {
  let schema = z.string();
...
export const createNumberSchema = (required: boolean = true, min?: number, max?: number) => {
  let schema = z.number();
...
export const createArraySchema = (itemSchema: z.ZodTypeAny, required: boolean = true) => {
