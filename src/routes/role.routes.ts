...
router.post('/', checkPermission('role.create'), validate(RoleCreateSchema), controller.create);
router.get('/', checkPermission('role.read'), controller.getAll);
router.get('/:id', checkPermission('role.read'), controller.getById);
router.put('/:id', checkPermission('role.update'), validate(RoleUpdateSchema), controller.update);
